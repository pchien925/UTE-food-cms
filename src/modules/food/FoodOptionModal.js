...
import FoodOptionForm from './FoodOptionForm';

const FoodOptionModal = ({ open, onCancel, foodId }) => {
    ...
    const { data, mixinFuncs, queryFilter, loading, pagination, serializeParams } = useListBase({
        apiConfig: apiConfig.foodOption,
        options: {
            pageSize: DEFAULT_TABLE_ITEM_SIZE,
            objectName: 'Food Option',
        },
        override: (funcs) => {
            ...
            funcs.getList = () => {
                ...
                mixinFuncs.handleFetchList({ ...params, foodId });
            };
    ...
    const {
        ...
        data: data,
        apiConfig: apiConfig.foodOption.updateSort,
        setTableLoading: () => {},
        indexField: 'ordering',
    });
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 100,
            render: (type) => {
                return type === 1 ? <Tag color="red">Bắt buộc</Tag> : <Tag color="blue">Tùy chọn</Tag>;
            },
        },
        {
            title: 'Chọn tối đa',
    ...
    return (
        <Modal
            title="Danh sách tùy chọn (Option)"
            open={open}
            onCancel={onCancel}
            width={1000}
            footer={null}
