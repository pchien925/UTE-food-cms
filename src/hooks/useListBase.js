...
import { Modal, Button, Divider, Tag, Flex } from 'antd';
...
import Empty from '@components/common/elements/Empty/Empty';
import useOnScreen from './useOnScreen';

import styles from './useListBase.module.scss';
...
const useListBase = ({
                ...
                executeGetList({
                    onCompleted: (res) => {
                        setPagination((p) => ({
                            ...
                            current: p.current < p.totalPages ? p.current + 1 : p.totalPages,
                        }));
                    },
                    onError: (error) => {
    ...
    const RenderListMobile = ({
        ...
        useEffect(() => {
            if (isIntersecting && hasMore && !loadingMobile) {
                loadMore();
                observer.disconnect();
            }
        }, [isIntersecting, hasMore, loadMore, loadingMobile]);
        ...
        return (
            <>
                <SwipeableList threshold={0.5} type={ListType.IOS} fullSwipe={false} className={styles.swipeableList}>
                    {data?.map((item, index) => (
                        <SwipeableListItem
                            actionDelay={3000}
                            key={index}
                            blockSwipe={!isSwipeable ? true : handleSwipeItem(item)}
                            className={`${styles.swipeableListItem}`}
                            //trailingActions={mixinFuncs.renderActionButtonSwipe(item)}
                        >
                            {renderItem(item, index, measureRef, data.length)}
                        </SwipeableListItem>
                    ))}
                </SwipeableList>
                <LoadingWrapper loading={loadingMobile} />
            </>
        );
    };
